...
void ElementSystem::AllenCahn(const int &iState, const int (&IX)[27], const int &nDim, const int &nNodes,
...
{
    //******************************************
    //*** The AllenCahn equation looks like:
    //*** du/dt=lap(u)-f(u)/eps^2
    //******************************************
    int i,j,k;
    int ngp,Lint,gpInd;
    int iInd,jInd;
    ...
    for(gpInd=0;gpInd<Lint;gpInd++)
    {
        ...
        gradc[1]=0.0;gradc[2]=0.0;gradc[3]=0.0;
        conc=0.0;cdot=0.0;
        for(i=0;i<nNodes;i++)
        {
            conc+=shp[i][0]*U[i][0];
            ...
            for(k=1;k<=nDim;k++)
            {
                gradc[k]+=shp[i][k]*U[i][0];
            }
        }


        FreeEnergyForAllenCahn(conc,f,dfdc,d2fdc2);
        ...
        if(iState%3==0)
        {
            ...
            {
                // R_c
                rhs[iInd  ]+=cdot*shp[iInd][0]*JxW
                            +eps*eps*dfdc*shp[iInd][0]*JxW;

                for(k=1;k<=nDim;k++)
                {
                    // For gradient term
                    // R_c
                    rhs[iInd]+=gradc[k]*shp[iInd][k]*JxW;
                ...
                if(iState==6)
                {
                    ...
                    {
                        // Kc,cdot
                        K[iInd*nDofs+jInd]+=-shp[jInd][0]*shp[iInd][0]*JxW*ctan[1];

                        // Kc,c
                        K[iInd*nDofs+jInd]+=-eps*eps*d2fdc2*shp[jInd][0]*shp[iInd][0]*JxW*ctan[0];
                        for(k=1;k<=nDim;k++)
                        {
                            // Kc,mu
