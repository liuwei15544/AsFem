...
// for mesh block reader

#include "InputSystem/InputSystem.h"

bool InputSystem::ReadMeshBlock(Mesh &mesh)
{
    // the mesh block should looks like:
    // [mesh]
    //   type=gmsh
    //   filename=
    // [end]
    ...
    if(IsBracketMatch(in,line0,blockstartlinenum))
    {
        ...
        while(str.compare(0,2,"[]")!=0)
        {
            ...
            if(str.compare(0,10,"type=asfem")==0)
            {
                ...
                {
                    Msg_InputFile_MeshBlock_NoDim(linenum);
                    return false;
                }
                numbers=SplitNum(line);
                if(numbers.size()<1)
                {
                    Msg_InputFile_NoNumberFound(linenum);
                    return false;
                }
                else
                {
                    ...
                    if(dim<1||dim>3)
                    {
                        Msg_InputFile_MeshBlock_DimInvalid(linenum);
                        return false;
                    }
                    else if(dim==1)
                    {
                        ...
                        {
                            Msg_InputFile_MeshBlock_XminNotFound(linenum);
                            return false;
                        }
                        ...
                        {
                            Msg_InputFile_MeshBlock_XmaxNotFound(linenum);
                            return false;
                        }
                    ...
                    else if(dim==3)
                    {
                        ...
                        {
                            Msg_InputFile_MeshBlock_YminNotFound(linenum);
                            return false;
                        }
